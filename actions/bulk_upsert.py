...
def main() -> None:
    ...
    ext = params.get("external_id_field")
    try:
    ...
    if not sobject or not ext or not records:
        sf_client.fail(
            "missing_param: sobject, external_id_field and a non-empty "
            "records source (records[] or input_artifact_ref/input_artifact_id) required",
        )
